...
class AClass_OrderList extends AClass_FormBase {
    protected $user = null;
    protected $stations_overview = null;
    protected $post_variants = null;
    protected $auslieferungsDivId = 0;
    protected $auslieferungsPools = null;
    ...
    const LOCAL_CSS = "
            h3                                  {margin-bottom: 0; position: relative; top: -10px;}
            ...
            .orders tr                          { }
            .orders td                          {overflow:hidden; float: left; }
            /*
            ...
            .orders tr td:nth-child(11)         {width: 10px;  padding: 0px 0; border-left: none; }
            */
            .orders input                       {margin: 0;}
            #id_orders_head tr                  {background-color: #ccc; }
            #id_orders_head td                  {border: 1px solid #444; height: 36px }

            #id_orders_body tr                  {background-color: #fff; cursor: pointer; }
            #id_orders_body tr.selected         {background-color: #ccf; }
            #id_orders_body td                  {border: 1px dotted #ccc; height: 24px;  }
...
HEREDOC;
    ...
    function Init() {
        ...
        $qry = $this->leitWartePtr->newQuery('divisions')->where('name', 'ilike', '%post%ausliefer%');
        $this->auslieferungsDivId = $qry->getVal('division_id');


        $qry = $this->leitWartePtr->newQuery('depots');
        $qry = $qry->where('division_id', '=', $this->auslieferungsDivId)->orderBy('name');
        $this->auslieferungsPools = $qry->get('*', 'depot_id');
...
HEREDOC;
    ...
    //#########################################################################
    function QueryVehiclesInPool($depot, $where = null, $nur_angemeldete = true) {
        $qry = $this->vehiclesPtr->newQuery();
        $qry = $qry->join('vehicle_variants', 'vehicle_variant=vehicle_variant_id');
        if (is_array($depot))
            $qry = $qry->where('depot_id', 'in', $depot);
        else
            $qry = $qry->where('depot_id', '=', $depot);

        if (isset ($where)) {
            if (isset($where['type']))
                $qry = $qry->where('vehicle_variants.type', '=', $where['type']);
            if (isset($where['seats']))
                $qry = $qry->where('vehicle_variants.number_of_seats', '=', $where['seats']);
            if (isset($where['battery'])) {
                $op = is_array($where['battery']) ? 'in' : '=';
                $qry = $qry->where('vehicle_variants.battery', $op, $where['battery']);
            }
        }
        if ($nur_angemeldete) {
            $qry = $qry->where('code', 'is not null');
            $qry = $qry->where('ikz', 'is not null');
        }

        return safe_array($qry->get('vehicle_id, code, ikz, vin, vehicle_variant, windchill_variant_name, type, number_of_seats, battery', 'vehicle_id'));
    }
    ...
    function Execute() {
        ...
        $this->actual_orders = $this->QueryActualOrders();
    }
            ...
            <div id="id_orders_head">
                <table class="transparent orders">
                    <tr>
                        ...
                        <td>OZ</td>
                        <td>Lade-station</td>
                        <td>Fahrzeug<br>Typ</td>
                        <td>Batterie</td>
                        <td>Anzahl<br>Sitze</td>
                        <td>Bestelldatum</td>
                        <td>Auslieferung<br>(geplant)</td>
                        <td>&nbsp;</td>
                    </tr>
                </table>
            </div>
            <div id="id_orders_body">
                <table class="transparent orders" id="ordertable">
                    <?php
