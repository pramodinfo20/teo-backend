<?php
/**
 * CoCParametersController.class.php
 * @author Grzegorz Stolarz, FEV
 */
$GLOBALS['search_hide_fieldset'] = true;

class CoCParametersController extends PageController {
    const METHOD_LIST = 'list';
    const METHOD_NEW = 'new';
    const METHOD_EDIT = 'edit';
    const METHOD_DELETE = 'delete';
    const METHOD_SAVE_NEW = 'saveNew';
    const METHOD_SAVE_EDITED = 'saveEdited';
    ...
    private static $allowedActions = [
        self::METHOD_LIST => ['method' => 'listCoCParameters', 'printContent' => true],
        self::METHOD_NEW => ['method' => 'newCoCParameter', 'printContent' => true],
        self::METHOD_EDIT => ['method' => 'editCoCParameter', 'printContent' => true],
        self::METHOD_DELETE => ['method' => 'deleteCoCParameter', 'printContent' => true],
        self::METHOD_SAVE_NEW => ['method' => 'saveNewCoCParameter', 'printContent' => false],
        self::METHOD_SAVE_EDITED => ['method' => 'saveEditedCoCParameter', 'printContent' => false]
    ];
    private $types;
    private $msgs;
    ...
    private function listCoCParameters() {
        $this->view = $this->getMiddleware()
            ->prepare(Middleware::REQUEST_TYPE_GET, 'parameters/coc')
            ->sentRequest();
    }

    //todo: FIX USER_ID, SESSION MECHANISM
    private function newCoCParameter() {
        if ($_SERVER['REQUEST_METHOD'] == Middleware::REQUEST_TYPE_GET) {
            $this->view = $this->getMiddleware()
                ->prepare(Middleware::REQUEST_TYPE_GET, 'parameters/coc/new/'.$_SESSION['sts_userid'])
                ->sentRequest();
        } elseif ($_SERVER['REQUEST_METHOD'] == Middleware::REQUEST_TYPE_POST) {
            $response = $this->getMiddleware()
                ->prepare(Middleware::REQUEST_TYPE_POST, 'parameters/coc/new/'.$_SESSION['sts_userid'])
                ->sentRequest();

            $json = json_decode($response);

            if ($json->status == 302) {
                header('Location: ' . $this->getMiddleware()->removeUrlQuery($json->redirect, 'cocParameter'));
                exit;
            }
        }
    }

    private function editCoCParameter() {
        $cocParameterId = (isset($_GET['cocParameter']) && $_GET['cocParameter'])
            ? intval($_GET['cocParameter'])
            : 0;

        if ($_SERVER['REQUEST_METHOD'] == Middleware::REQUEST_TYPE_GET) {
            $this->view = $this->getMiddleware()
                ->prepare(Middleware::REQUEST_TYPE_GET, "parameters/coc/{$cocParameterId}/edit")
                ->sentRequest();
        } elseif ($_SERVER['REQUEST_METHOD'] == Middleware::REQUEST_TYPE_POST) {
            $response = $this->getMiddleware()
                ->prepare(Middleware::REQUEST_TYPE_POST, "parameters/coc/{$cocParameterId}/edit")
                ->sentRequest();
    ...
    private function saveEditedCoCParameter() {
        $cocParameterId = (isset($_GET['cocParameter']) && $_GET['cocParameter'])
            ? intval($_GET['cocParameter'])
            : 0;

        $response = $this->getMiddleware()
            ->prepare(Middleware::REQUEST_TYPE_POST, "parameters/coc/{$cocParameterId}/edit/save")
            ->sentRequest();

        echo $response;
    }

    private function saveNewCoCParameter() {
        $response = $this->getMiddleware()
            ->prepare(Middleware::REQUEST_TYPE_POST, 'parameters/coc/new/'.$_SESSION['sts_userid'].'/save')
            ->sentRequest();

        echo $response;
    }

    public function deleteCoCParameter() {
        $cocParameterId = (isset($_GET['cocParameter']) && $_GET['cocParameter'])
            ? intval($_GET['cocParameter'])
            : 0;

        $response = $this->getMiddleware()
            ->prepare(Middleware::REQUEST_TYPE_DELETE, "parameters/coc/{$cocParameterId}")
            ->sentRequest();
    ...
    public function printContent() {
        ...
        include("pages/cocParameters.php");
