...
class QsController extends PageController {
    /**
     * @var CommonFunctions_QsFaultSearch
     */
    protected $qs_fault_search;

    /**
     * @var CommonFunctions_TeoSearch
     */
    protected $teo_search;
    ...
    public function setupDepots() {
        ...
        $this->depotFilterContent = '<form action="index.php" method="POST">
    ...
    public function setupHeaders() {
        ...
        if ($this->production_depots_only)
            $delivery_status_params = ['id' => 'finished_status_search_ctrl', 'data-placeholder' => "t/f&nbsp;eingeben", 'data-sorter' => 'false'];
            else
            $delivery_status_params = ['id' => 'finished_status_search_ctrl', 'data-filter' => 'false', 'data-sorter' => 'false'];
        ...
        $this->headers = ['vin' => ['VIN'],
            ...
            'sub_vehicle_configuration_name' => ['Subkonfiguration'],
            'short_production_description' => ['Produktionskurzbezeichnung'],
            'delivery_status' => ['Ausgeliefert', $delivery_status_params],
            ...
            'production_date' => ['Production Datum', ['data-filter' => 'false', 'data-sorter' => 'false']],
    ...
    public function ajaxGetQSFaultForm() {
        ...
        $form_elements_html = '<form action="index.php" id="qs_faults_list" method="POST"><table class="qs_faults_list_wrap">';
...
HEREDOC;
            ...
            } else if ($vehicle['delivery_status'] == 'f') {
...
HEREDOC;
    ...
    public function getDataSingleVehicle($vehicle_id) {
        $vehicle = $this->ladeLeitWartePtr->vehiclesPtr->newQuery()
            ...
            ->where('vehicle_id', '=', $vehicle_id)
            ->getOne("vehicles.vehicle_id,vin,vehicles.ikz,code,penta_kennwort,c2cbox,vehicles.vehicle_variant,windchill_variant_name,vehicles_sales.production_date,vehicles_sales.delivery_status,
            depots.name as dname,depots.depot_id,finished_status,qmlocked,special_qs_approval,vehicles.penta_number_id,penta_number,
    ...
    public function getVehicleVins() {
        if (isset($_POST['start_date']) && !empty($_POST['start_date'])) {
         ...
         }  else if (isset($_POST['start_prod']) && !empty($_POST['start_prod'])) {
            $start_prod = filter_var($_POST['start_prod'], FILTER_SANITIZE_STRING);

            $result = $this->ladeLeitWartePtr->newQuery('vehicles_sales')->join('vehicles', 'vehicles.vehicle_id=vehicles_sales.vehicle_id', 'INNER JOIN')->join('latest_teo_status', 'latest_teo_status.teo_vin=vehicles.vin', 'INNER JOIN')
                ->where('production_date', '>=', $start_prod);
            if (isset($_POST['end_prod']) && !empty($_POST['end_prod'])) {
                $end_prod = filter_var($_POST['end_prod'], FILTER_SANITIZE_STRING);
                $result = $result->where('production_date', '<=', $end_prod);
            }

            $vins = $result->orderBy('production_date', 'ASC')->get('teo_vin');
