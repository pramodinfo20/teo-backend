...
<html lang="de">
<head>
<meta http-equiv="refresh" content="10; URL=https://streetscooter-cloud-system.eu/end-of-line-test/failed">
<meta charset="UTF-8">
...
//$query = "select vin,v.c2cbox,update_timestamp,CASE WHEN sd_card = 'ok' AND reverse_tunnel = 'ok' AND vin_ok = 'ok' AND can_ok = 'ok' THEN 1 WHEN sd_card IS NULL AND reverse_tunnel IS NULL AND vin_ok IS NULL AND can_ok IS NULL THEN 0 ELSE -1 END as checked from vehicles as v LEFT JOIN c2c_configuration as c on v.c2cbox = c.c2cbox WHERE substring(vin,1,5) = 'WS5B1' AND substring(vin,12,5)!='MIRCO' AND v.c2cbox IS NOT NULL ORDER BY substring(vin,1,6) DESC, substring(vin,13) DESC LIMIT 3000;";
