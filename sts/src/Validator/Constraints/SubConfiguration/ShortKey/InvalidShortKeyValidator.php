...
class InvalidShortKeyValidator extends ConstraintValidator
{
    private $manager;
    ...
    {
        if (!$constraint instanceof InvalidShortKey) {
            throw new UnexpectedTypeException($constraint, InvalidShortKey::class);
        }
        ...
        if (is_null($key)) {
            ...
            $subConfigurations = $this->manager->getRepository(VehicleConfigurations::class)
                ->findDuplicatedInvalidShortKey($key);
